...
function renderGuide() {
  ...
  appRoot.innerHTML = `${backButton()}<section class="auramax-page-head"><p class="eyebrow">AURAMAX PREMIUM PLAYBOOK</p><h2>Your complete 30-day looksmax system.</h2><p>A structured manual for grooming, skin, hair, body, style, photos and presence. Work through one practical lesson at a time—then keep the habits that fit your real life.</p><div class="guide-value-summary"><span><b>${chapters.length}</b> premium chapters</span><span><b>${lessonCount}</b> practical lessons</span><span><b>30 days</b> to build your system</span></div></section>${learnGalleryMarkup('Looksmax Complete Guide')}<section class="guide-chapter-directory" aria-label="AuraMax Complete Looksmax Playbook chapters"><div class="guide-directory-head"><div><p class="eyebrow">THE COMPLETE PLAYBOOK</p><h3>Choose your next chapter</h3></div><span>${chapters.length} chapters</span></div><div class="chapter-grid">${chapters.map(chapter => `<button type="button" class="chapter-card" data-aura-guide-chapter="${escapeHtml(chapter.id)}"><small>CHAPTER ${escapeHtml(chapter.tag)}</small><h3>${escapeHtml(chapter.name)}</h3><p>${escapeHtml(chapter.desc)}</p><span class="guide-card-action">Open chapter <b aria-hidden="true">→</b></span></button>`).join('')}</div></section>`;
}
