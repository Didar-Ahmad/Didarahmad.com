...
function renderGuideLesson(chapterId, index) {
  ...
  appRoot.innerHTML = `<article class="guide-reader"><button type="button" class="guide-back-button" data-aura-guide-back="chapter" data-aura-guide-chapter-id="${escapeHtml(chapter.id)}"><span aria-hidden="true">←</span> Back to ${escapeHtml(chapter.name)}</button><header><p class="eyebrow">CHAPTER ${escapeHtml(chapter.tag)} · LESSON ${String(lessonIndex + 1).padStart(2, '0')}</p><h1>${escapeHtml(title)}</h1><p class="guide-reader-lead">${escapeHtml(summary)}</p></header><section class="guide-reader-context"><p class="eyebrow">THE AURAMAX METHOD</p><h2>Build this into your real routine</h2><p>${escapeHtml(premium.principle)}</p><p>${escapeHtml(premium.focus)}</p></section><section class="guide-reader-steps"><p class="eyebrow">PRACTICAL STEPS</p><h2>Put the lesson into practice</h2><ol><li>${escapeHtml(firstStep || 'Start with the smallest comfortable action you can repeat.')}</li><li>${escapeHtml(secondStep || 'Keep the routine simple enough to follow consistently.')}</li><li>Review what feels useful after a week, then adjust one detail at a time.</li></ol></section><section class="guide-premium-grid"><section class="guide-premium-card"><p class="eyebrow">YOUR 7-DAY ROUTINE</p><h2>Use it this week</h2><ol>${premium.routine.map(step => `<li>${escapeHtml(step)}</li>`).join('')}</ol></section><section class="guide-premium-card guide-mistakes"><p class="eyebrow">COMMON MISTAKES</p><h2>Choose the better move</h2>${premium.mistakes.map(([avoid, instead]) => `<div><strong>Avoid: ${escapeHtml(avoid)}</strong><span>Do instead: ${escapeHtml(instead)}</span></div>`).join('')}</section></section><section class="guide-premium-examples"><div><p class="eyebrow">REAL-LIFE EXAMPLES</p><h2>What good progress looks like</h2></div><ul>${premium.examples.map(example => `<li>${escapeHtml(example)}</li>`).join('')}</ul></section><section class="guide-checklist"><div><p class="eyebrow">SAVE THIS CHECKLIST</p><h2>Before you move to the next lesson</h2></div><ul>${premium.checklist.map(item => `<li><span aria-hidden="true">✓</span>${escapeHtml(item)}</li>`).join('')}</ul></section><aside class="guide-reader-note"><strong>Healthy, practical and personal.</strong><span>Use this guide for healthy grooming, style, confidence and habits—not unsafe appearance changes. Comfort, health and consistency matter more than chasing a perfect result.</span></aside><nav class="guide-reader-navigation" aria-label="Lesson navigation"><button type="button" ${previous === null ? 'disabled' : `data-aura-guide-navigate="${previous}" data-aura-guide-chapter-id="${escapeHtml(chapter.id)}"`}>← Previous lesson</button><button type="button" data-aura-guide-back="chapter" data-aura-guide-chapter-id="${escapeHtml(chapter.id)}">All lessons</button><button type="button" ${next === null ? 'disabled' : `data-aura-guide-navigate="${next}" data-aura-guide-chapter-id="${escapeHtml(chapter.id)}"`}>Next lesson →</button></nav></article>`;
  appRoot.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
